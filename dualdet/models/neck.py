"""PAN-FPN detection necks for fused feature pyramids."""
...
class PANFPN(nn.Module):
    """Aggregate fused P3/P4/P5 features with top-down and bottom-up paths."""
    ...
    def _validate_channel_config(cls, feature_channels: Mapping[str, int]) -> None:
        ...
        if any(feature_channels[level] <= 0 for level in cls.input_feature_names):
...
class P2PANFPN(nn.Module):
    """P2-enhanced PAN-FPN using explicit stride-4 input features.

    This is the member-3 E3 path: P2 comes from the shallow dual-stream
    backbone feature and is fused before entering the neck.
    """
    ...
    def __init__(self, feature_channels: Mapping[str, int], repeats: int = 1) -> None:
        ...
        c2, c3, c4, c5 = (feature_channels[level] for level in self.feature_names)
        self._feature_channels = dict(feature_channels)

        self.p5_reduce = ConvBNAct(c5, c4, kernel_size=1)
        self.p4_top_down = C2f(c4 + c4, c4, repeats)
        self.p4_reduce = ConvBNAct(c4, c3, kernel_size=1)
        self.p3_top_down = C2f(c3 + c3, c3, repeats)
        self.p3_reduce = ConvBNAct(c3, c2, kernel_size=1)
        self.p2_out = C2f(c2 + c2, c2, repeats)

        self.p2_downsample = ConvBNAct(c2, c3, kernel_size=3, stride=2)
        self.p3_out = C2f(c3 + c3, c3, repeats)
        self.p3_downsample = ConvBNAct(c3, c3, kernel_size=3, stride=2)
        self.p4_out = C2f(c3 + c4, c4, repeats)
        self.p4_downsample = ConvBNAct(c4, c4, kernel_size=3, stride=2)
    ...
    def _validate_channel_config(cls, feature_channels: Mapping[str, int]) -> None:
        ...
        actual = set(feature_channels)
        if actual != expected:
            raise ValueError("feature_channels must contain exactly p2, p3, p4 and p5")
    ...
    def _validate_features(self, features: Mapping[str, Tensor]) -> None:
        ...
        if set(features) != expected:
            raise ValueError("features must contain exactly p2, p3, p4 and p5")
        ...
        p2, p3, p4, p5 = (features[level] for level in self.feature_names)
        if len({p2.shape[0], p3.shape[0], p4.shape[0], p5.shape[0]}) != 1:
            raise ValueError("all feature levels must have the same batch size")
        if p2.shape[2] != p3.shape[2] * 2 or p2.shape[3] != p3.shape[3] * 2:
            raise ValueError("p2 spatial size must be exactly twice p3")
        if p3.shape[2] != p4.shape[2] * 2 or p3.shape[3] != p4.shape[3] * 2:
            raise ValueError("p3 spatial size must be exactly twice p4")
        if p4.shape[2] != p5.shape[2] * 2 or p4.shape[3] != p5.shape[3] * 2:
            raise ValueError("p4 spatial size must be exactly twice p5")
        if len({p2.device, p3.device, p4.device, p5.device}) != 1:
            raise ValueError("all feature levels must be on the same device")
        if len({p2.dtype, p3.dtype, p4.dtype, p5.dtype}) != 1:
            raise ValueError("all feature levels must have the same dtype")
    ...
    def forward(self, features: Mapping[str, Tensor]) -> FeaturePyramid:
        """Return context-enhanced P2/P3/P4/P5 features."""
        ...
        p4_reduced = self.p4_reduce(p4_td)
        p3_td = self.p3_top_down(
            torch.cat((p3, F.interpolate(p4_reduced, size=p3.shape[2:], mode="nearest")), dim=1)
        )
        p3_reduced = self.p3_reduce(p3_td)
        p2_out = self.p2_out(
            torch.cat((p2, F.interpolate(p3_reduced, size=p2.shape[2:], mode="nearest")), dim=1)
        )

        p3_out = self.p3_out(torch.cat((self.p2_downsample(p2_out), p3_td), dim=1))
        p4_out = self.p4_out(torch.cat((self.p3_downsample(p3_out), p4_td), dim=1))
        p5_out = self.p5_out(torch.cat((self.p4_downsample(p4_out), p5), dim=1))
...
class PANFPNWithP2(PANFPN):
    """Compatibility neck that derives a lightweight P2 output from P3.

    This keeps member-2's training builder callable while the canonical E3
    model can still use :class:`P2PANFPN` when explicit P2 backbone features
    are available.
    """
    ...
    def forward(self, features: Mapping[str, Tensor]) -> FeaturePyramid:
        ...
        outputs = {"p2": self.p2_out(outputs["p3"]), **outputs}
        return outputs

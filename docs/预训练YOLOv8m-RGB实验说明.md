# 预训练 YOLOv8m RGB/TIR 实验说明

## 1. 实验目的

自研四模型融合线上 mAP 为 `0.278432`。新增实验使用开源 COCO 预训练 `yolov8m.pt` 分别训练 RGB 和 TIR 单模型，并依据线上结果选择有效模态加入最终融合。
...
融合后参数量必须填写所有实际参与推理模型参数量之和，不能继续沿用原四模型的 `52.91M`。
...
若 JSON 超过 5MB，依次提高 `--conf` 至 `0.005`、`0.006`，并保证输出文件名中的阈值与实际参数一致。

## 10. 实际结果与最终融合

- RGB YOLOv8m：验证集 COCO JSON 回放 AP `0.444`，线上 mAP `0.090413`，不参与最终融合；
- TIR YOLOv8m：本地最佳验证 mAP `0.63796`，线上单模型最佳 mAP `0.327517`，参数量 `25.84M`；
- 原四模型阶段性融合：线上 mAP `0.278432`，参数量 `52.91M`；
- 最终五模型融合：线上 mAP `0.357833`，参数量 `78.75M`。

最终文件为 `outputs/result_final.json`。融合时保留 TIR 预测权重 `1.0`、阈值 `0.002`，原四模型预测权重缩放为 `0.25`、阈值 `0.05`，随后按类别执行 `NMS IoU=0.68`。最终结果相对 TIR 单模型提升 `0.030316`，相对原四模型提升 `0.079401`。

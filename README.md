...
- `scripts/convert_coco_to_yolo_rgb.py`、`train_ultralytics_rgb.py`、`infer_ultralytics_rgb.py`：开源 COCO 预训练 YOLOv8m RGB 扩展实验链路。
- `docs/成员3-P2小目标检测头与指标分析说明.md`：成员 3 的 P2 Head 与指标分析说明。

预训练 RGB 扩展实验的完整命令见 `docs/预训练YOLOv8m-RGB实验说明.md`。该实验独立写入 `outputs/ultralytics/`，不会覆盖当前冻结的四模型提交结果。

## 最终提交结果

最终采用四模型结果级融合，而不是单独采用 E2：

- 单模型线上最佳：E1 fixed fusion；
- 最终 JSON：`outputs/result_ens4_nms68_s003_compact.json`；
- 融合组成：E1 fixed fusion（v8s，NMS=0.8）+ E1 fixed fusion（v8m，NMS=0.8）+ E2 QAF + E3 QAF+P2；
- 融合后处理：按类别执行 NMS，`NMS IoU=0.68`，`score_threshold=0.003`；
- 模型大小：`52.91M`；
- 最终线上 mAP：`0.278432`。

...
最终采用三模型结果级融合，而不是单独采用 E2：

- 单模型线上最佳：E1 fixed fusion；
- 最终 JSON：`outputs/result_e1nms08_e2_e3_nms70_s002_compact.json`；
- 融合组成：E1 fixed fusion（NMS=0.8）+ E2 QAF + E3 QAF+P2；
- 融合后处理：按类别执行 NMS，`NMS IoU=0.70`，`score_threshold=0.002`；
- 模型大小：`27.71M`；
- 最终线上 mAP：`0.271375`。
